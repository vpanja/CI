function set_order_status(target_url, order_num, orderStatus)
{
    $('#myModal').modal();
    $.ajax({
        mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
        url: siteurl + target_url,
        data: 'order_num=' + order_num + '&status=' + orderStatus,
        type: 'POST',
        dataType: "html",
        async: false,
...
function product_popup(target_url, pro_num)
{
    $('#myModal').modal();
    $.ajax({
        mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
        url: siteurl + target_url,
        data: 'product_id=' + pro_num,
        type: 'POST',
        dataType: "html",
        async: false,
        success: function(data) {
...
function show_popupview(target_url)
{
    $('#myModal').modal();
    $.ajax({
        mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
...
function DrawAllCharts() {
    //  Chart 1
    var chart1_data = chartInputData;
    var chart1_options = {
...
function LoadTestMap() {
    $.getJSON("http://www.telize.com/geoip?callback=?",
            function(json) {
                var osmap = new OpenLayers.Layer.OSM("OpenStreetMap");//�?оздание �?ло�? карты
                ...
                var osmap1 = new OpenLayers.Layer.OSM("OpenStreetMap");//�?оздание �?ло�? карты
